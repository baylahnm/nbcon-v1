/**
 * StatusCards - Advanced Stats Cards with Expandable Modals
 * 
 * Features:
 * - Live Supabase API data
 * - Expandable full-screen modals with Framer Motion
 * - Full LineChart on expand
 * - Detailed analytics breakdowns
 * - Bauhaus gradient animated hover
 * 
...
import { 
  ...
  ChartTooltipContent,
  ChartConfig
} from '@/pages/1-HomePage/others/components/ui/line-chart';
 ...
 * 
 * Advanced stats cards with live Supabase data and expandable modals
 */
export function StatusCards() {
  ...
  useEffect(() => {
    async function loadMetrics() {
      ...
      try {
        ...
        console.error('[StatusCards] Error loading overview metrics:', err);
  ...
  const statsCards = metrics ? [
    ...
    },
    {
      ...
      description: "Last 6 months trend",
      chartData: metrics.totalEngineers.chartData.map(d => ({
        month: d.month || '',
        value: d.value || 0
      })),
      expandedContent: () => (
    ...
    },
    {
      ...
      description: "Last 6 months trend",
      chartData: metrics.pendingQuotes.chartData.map(d => ({
        month: d.month || '',
        value: d.value || 0
      })),
      expandedContent: () => (
    ...
    },
    {
      ...
      description: "Last 6 months trend",
      chartData: metrics.totalSpent.chartData.map(d => ({
        month: d.month || '',
        value: d.value || 0
      })),
      expandedContent: () => (
        ...
        {active ? (
          <div className="fixed inset-0 grid place-items-center z-[100]">
            <motion.button
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0, transition: { duration: 0.05 } }}
              className="flex absolute top-2 right-2 lg:hidden items-center justify-center bg-background rounded-full h-6 w-6 shadow-lg"
