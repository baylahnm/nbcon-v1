export function JobsNearYou({ jobs, onOpen }:{
  ...
  return (
    <ul className="space-y-2">
      {jobs.map(j=>(
        <li key={j.id} className="rounded-lg border border-[var(--border)] p-3 flex items-center justify-between" style={{boxShadow: 'inset 4px 4px 4px hsl(var(--foreground)/0.06), inset -4px -4px 4px hsl(var(--foreground)/0.03)'}}>
