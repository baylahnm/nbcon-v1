...
export function MessagesContent() {
  ...
  const getFilteredEmojis = () => {
    ...
    if (emojiCategory === "all") {
      ...
      emojis = (emojiData as any)[emojiCategory] || [];
    }
        ...
        <ScrollArea className="flex-1 p-4 bg-muted/30">
          <div className="space-y-4 max-w-4xl mx-auto">
            ...
            {messages.map((message: HookMessage) => {
              ...
              const originalFileName = (message as any).fileName ?? (message as any).file_name ?? message.file_url?.split('/')?.pop() ?? '';
              const displayFileName = originalFileName || 'Attachment';
              const lowerFileName = originalFileName.toLowerCase();
              const fileSize = (message as any).fileSize ?? (typeof (message as any).file_size === 'number' ? (message as any).file_size : undefined);
                  ...
                  {!isCurrentUser && (
                    <Avatar className="w-8 h-8">
                      <AvatarFallback className="bg-muted-foreground text-muted-foreground text-xs">
                        {(message as any).sender_profile?.first_name ? ((message as any).sender_profile?.first_name as string).charAt(0) : '?'}
                      </AvatarFallback>
                    </Avatar>
                  )}
                  ...
                  <div className={`max-w-lg ${isCurrentUser ? 'items-end' : 'items-start'} flex flex-col`}>
                    {!isCurrentUser && (
                      <div className="text-sm font-medium mb-1">
                        {`${(message as any).sender_profile?.first_name ?? ''} ${(message as any).sender_profile?.last_name ?? ''}`.trim()}
                      </div>
                    )}
                    ...
                    <div className={`rounded-lg px-4 py-2 ${
                      ...
                      {message.reactions && (message.reactions as any).length > 0 && (
                        <div className="flex gap-1 mt-2">
                          {(message.reactions as any).map((reaction: any, index: number) => (
                            <Button
                              key={index}
                              size="sm"
                              variant="secondary"
                    ...
                    <div className={`text-xs text-muted-foreground mt-1 ${isCurrentUser ? 'text-right' : ''}`}>
                      {formatTime(message.created_at as any)}
