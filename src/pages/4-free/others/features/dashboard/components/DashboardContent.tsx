...
export function DashboardContent() {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Main Content Area (Stitch-style layout) */}
      <div className="flex-1 flex flex-col overflow-hidden transition-all duration-300">
        
        {/* Top Header */}
        <header className="h-16 flex-shrink-0 border-b border-border/40 flex items-center justify-between p-4 bg-background" role="banner">
          <div className="flex items-center gap-4">
            <Avatar className="h-10 w-10 ring-2 ring-primary/20">
              ...
              <p className="text-xs text-muted-foreground">Your copilot for managing projects</p>
                  </div>
                </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => navigate('/free/ai')}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* AI Chat Conversation Area (Stitch-style) */}
          <div className={`flex-1 overflow-y-auto px-4 pt-4 pb-4 border-t border-border/40 ${activeMessages && activeMessages.length > 0 ? '' : 'flex items-center justify-center'}`} id="conversation-history">
            <div className={`w-full ${activeMessages && activeMessages.length > 0 ? 'space-y-4' : 'max-w-2xl mx-auto'}`}>
              {activeMessages && activeMessages.length > 0 ? (
                <>
                  ...
                  {/* Bottom padding for sticky composer */}
                  <div className="h-32" />
                </>
              ) : (
                /* Empty state - vertically & horizontally centered */
                <div className="text-center py-12 px-8 space-y-4 border border-border/40 rounded-lg bg-background/50">
                  <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                    <Bot className="h-8 w-8 text-primary" />
                  </div>
                  <div>
                    <p className="font-semibold text-foreground mb-2">Hello! How can I help you today?</p>
          ...
          {/* Sticky Bottom Composer (Stitch-style) */}
          <div className="sticky bottom-0 bg-background/95 backdrop-blur-md border-t border-border w-full z-10">
            <div className="w-full p-4">
              {/* Quick Action Pills (Horizontal Scroll with XScroll and Arrow Navigation) */}
              <div className="mb-4 relative dashboard-quick-actions-scroll">
                {/* Left fade/blur effect */}
                <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-background/95 to-transparent z-10 pointer-events-none" />
                
                {/* Right fade/blur effect */}
                <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-background/95 to-transparent z-10 pointer-events-none" />
                ...
                {showLeftArrow && (
                  <Button
                    size="icon"
                    variant="outline"
                    className="absolute left-2 top-1/2 -translate-y-1/2 z-20 h-8 w-8 rounded-full bg-background/98 backdrop-blur-md shadow-lg border-2 border-primary/20 hover:border-primary/40 hover:bg-primary/5 transition-all"
                ...
                {showRightArrow && (
                  <Button
                    size="icon"
                    variant="outline"
                    className="absolute right-2 top-1/2 -translate-y-1/2 z-20 h-8 w-8 rounded-full bg-background/98 backdrop-blur-md shadow-lg border-2 border-primary/20 hover:border-primary/40 hover:bg-primary/5 transition-all"
                ...
                <XScroll>
                  <div 
                    ...
                    className="flex space-x-4 p-1 pb-4"
                    style={{
                    ...
                    {QUICK_ACTIONS.map((action, index) => {
                      ...
                      return (
                        <div key={index} className="flex-shrink-0 snap-start">
                          <button
                            className="flex items-center gap-2 text-sm px-3 py-1.5 bg-background border border-border rounded-full hover:bg-accent transition-colors text-foreground whitespace-nowrap shadow-sm"
          ...
          {/* Recent Projects Section (Below Chat, Stitch-style) */}
            ...
            {projectsLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                {projects.slice(0, 6).map((project) => (
                  <div
                    key={project.id}
                    className="bg-card rounded-lg p-4 flex flex-col shadow-sm border border-border cursor-pointer hover:shadow-md hover:border-primary/50 transition-all"
                  ...
                  >
                    ...
                    <Badge 
                      variant="outline" 
                      className="inline-flex items-center text-xs bg-primary/10 text-primary border-primary/20 px-2 py-0.5 rounded-full mt-auto self-start"
                    >
                      <div className="w-2 h-2 mr-1.5 bg-primary rounded-full" />
                      {project.status}
                    </Badge>
                      </div>
                ))}
